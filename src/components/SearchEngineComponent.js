...
export class SearchEngine extends Component {
  ...
  handleOnKeyDown(event) {
    if (this.state.isSearching) {
      if (event.keyCode === 40) {
        if (this.state.activeResult < this.state.maxResults - 1)
        ...
        this.setState({
          stock:
            this.state.activeResult > 0
              ? this.state.filteredResults[this.state.activeResult - 1].symbol
              : this.state.filteredResults[this.state.activeResult].symbol,
          isLoading: true,
          description:
            this.state.activeResult > 0
              ? this.state.filteredResults[this.state.activeResult - 1]
                  .description
              : this.state.filteredResults[this.state.activeResult].description,
          delta: null,
          percentage: null,
          initialQuote: null,
          quote: null,
    ...
    if (stock !== "") {
      finnhubClient.quote(stock, (error, data, response) => {
        if (response.statusCode !== 200) {
          ...
          this.setState({
            delta:
              Math.round((data.c - initialQuote + Number.EPSILON) * 100) / 100,
            percentage:
              Math.round(
                ((data.c - initialQuote) / initialQuote + Number.EPSILON) * 100
              ) / 100,
          });
        }
      });
  ...
  render() {
    return (
      <React.Fragment className="center">
        <div
          className="col-12 col-lg-10  col-xl-8  searchbox-position"
          ...
          <ReactSearchAutocomplete
            ...
            placeholder={
              window.innerWidth >= 500
                ? `Get real-time US stock prices amongst the ${this.props.stockExchangeData.length.toLocaleString(
                    "en-US"
                  )} available`
                : `${this.props.stockExchangeData.length.toLocaleString(
                    "en-US"
                  )} stock prices`
            }
            styling={{
